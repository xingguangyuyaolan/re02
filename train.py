...
def main():
    ...
    reset_validation_retries = int(_get_value(cfg_file, "reset_validation_retries", 1))
    reset_validation_allow_failure = bool(_get_value(cfg_file, "reset_validation_allow_failure", False))
    min_spawn_pair_clearance = float(_get_value(cfg_file, "min_spawn_pair_clearance", 0.28))
    min_spawn_altitude = float(_get_value(cfg_file, "min_spawn_altitude", 0.14))
    spawn_layout_validation_retries = int(_get_value(cfg_file, "spawn_layout_validation_retries", 2))
    spawn_layout_allow_risky = bool(_get_value(cfg_file, "spawn_layout_allow_risky", False))
    ...
    qmix_cfg = QMIXConfig(
        ...
        safety_shield_emergency_dist=float(_get_value(cfg_file, "safety_shield_emergency_dist", 0.45)),
        safety_shield_lidar_dist=float(_get_value(cfg_file, "safety_shield_lidar_dist", 0.45)),
        safety_shield_boundary_margin=float(_get_value(cfg_file, "safety_shield_boundary_margin", 1.2)),
    ...
    env = GazeboMultiUAVParallelEnv(
        ...
        reset_validation_retries=reset_validation_retries,
        reset_validation_allow_failure=reset_validation_allow_failure,
        min_spawn_pair_clearance=min_spawn_pair_clearance,
        min_spawn_altitude=min_spawn_altitude,
        spawn_layout_validation_retries=spawn_layout_validation_retries,
        spawn_layout_allow_risky=spawn_layout_allow_risky,
    ...
    LOGGER.info(
        ...
        f"reset_validation_retries={reset_validation_retries} "
        f"reset_validation_allow_failure={reset_validation_allow_failure} "
        f"min_spawn_pair_clearance={min_spawn_pair_clearance} "
        f"min_spawn_altitude={min_spawn_altitude} "
        f"spawn_layout_validation_retries={spawn_layout_validation_retries} "
        f"spawn_layout_allow_risky={spawn_layout_allow_risky} "
